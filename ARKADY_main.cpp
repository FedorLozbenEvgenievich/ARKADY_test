...
int main() {
	...
	while (there==true) {
		...
		if (cond>=2) {printf(" 1<=[N=%5d]<=%d   cahnge->|",N,Nmax);}
		printf("\n");
		...
		if (cond==2) {
			if (ch=='w') {position_--;}
			if (ch=='s') {position_++;}
			if (position_==0) {position_=5;}
			if (position_==6) {position_=1;}
		}
		if (ch=='a') {
			if (cond==2) {cond=1;}
		}
		...
		if (start==true) {
			...
			for (int i=0;i<N;i++) {
				float ang=(rand()%10000)*2*pi/10000.0;
				Ob_dx[i]=v*cos(ang);
				Ob_dy[i]=v*sin(ang);
				Ob_x[i]=(rand()%10000)/10000.0*80;
				Ob_y[i]=(rand()%10000)/10000.0*100;
				Ob_cond[i]=0;
			...
			Ob_cond[0]=1;
			Ob_t[0]=t_distr[(int)((rand()%1000)/1000.0 * 100)]*24;
			...
			for (int t=0;t<T;t++) {
				...
				for (int i=0;i<N;i++) {
					if (Ob_cond[i]==0) {
									...
									for (int jj=0;jj<mas_is[xxx][yyy];jj++) {
										...
										if ((Ob_cond[j]==1)||(Ob_cond[j]==2)) {
											if (sq(Ob_x[i]-Ob_x[j])+sq(Ob_y[i]-Ob_y[j])<r*r) {
												if ((rand()%1000)/1000.0<p_sick) {Ob_cond[i]=1;}
											}
										}
									}
								}
						...
						if (Ob_cond[i]==1) {
							Ob_t[i]=t_distr[(int)((rand()%1000)/1000.0 * 100)]*24;
						}
					} else {
						if (Ob_cond[i]!=3) {
							if ((rand()%1000)/1000.0<p_heal) {
								Ob_cond[i]=0;
